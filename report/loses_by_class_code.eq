description Claim Developement by Policy Year - SUMMARY Version;

include "startend.inc"

define unsigned ascii number l_pol_year[4] = year(lrssetup:eff_date)
define file cppgenerala = access cppgeneral, set cppgeneral:policy_no    = lrssetup:policy_no,
                                                 cppgeneral:pol_year     = l_pol_year, generic 

define file cppliabilitya = access cppliability, set cppliability:policy_no    = lrssetup:policy_no,
                                                     cppliability:pol_year     = l_pol_year, generic 

define unsigned ascii number l_policy_year[4]=year(lrssetup:eff_date)

where 1 = 1
/*(((lrsdetail:loss_paid <> 0 and
        (lrsdetail:trans_date => l_starting_date and
         lrsdetail:trans_date <= l_ending_date)) or

        (lrsdetail:lae_paid <> 0 and
        (lrsdetail:trans_date => l_starting_date and
         lrsdetail:trans_date <= l_ending_date)) or

        ((lrsdetail:loss_resv <> 0 and
         (lrssetup:status <> "C") or
         (lrssetup:status = "C" and
          lrssetup:status_date => l_starting_date))) or

        ((lrsdetail:lae_resv <> 0 and
         (lrssetup:status <> "C") or
        (lrssetup:status = "C" and
         lrssetup:status_date => l_starting_date)))))
*/
and lrsdetail:trans_code not one of 91,92,93,94,95,96,97,98,99

and lrsdetail:trans_code < 30 
and lrsdetail:trans_date => l_starting_date 
and lrsdetail:trans_date <= l_ending_date 
--and lrssetup:agent_no one of 111
--and year(lrssetup:eff_date ) = 2007
and (lrsdetail:loss_paid <> 0 or 
     lrsdetail:loss_resv <> 0 or 
     lrsdetail:lae_paid <> 0 )
and l_policy_year > 2001
and lrssetup:line_of_business one of 8, 24 and 
(cppgenerala:class_code one of 18,3 or 
 cppliabilitya:class_code one of 1018,2018,1003,2003)

list
/title="Claim Developement by Policy Year - SUMMARY Version"
/nobanner
/domain="lrsdetail"
/nodetail 
/noreporttotals 

lrsdetail:claim_no /column=1
lrssetup:policy_no/column=20 
lrssetup:agent_no /column=35
lrssetup:line_of_business/heading="LOB" /column=40
lrssetup:loss_date/column=50
--cppgenerala:class_code/heading="Prop-Class"/column=60
--cppliabilitya:class_code/heading="Liab-Class"/column=80
lrsdetail:loss_resv/mask="(ZZZ,ZZZ,ZZZ.99)"/column=100 
lrsdetail:loss_paid/mask="(ZZZ,ZZZ,ZZZ.99)" /column=120
lrsdetail:lae_resv/mask="(ZZZ,ZZZ,ZZZ.99)" /column=140
lrsdetail:lae_paid/mask="(ZZZ,ZZZ,ZZZ.99)"/column=160
""/heading="NET"/column=190

sorted by --l_policy_year
           lrsdetail:claim_no--/newlines

/*top of l_policy_year 
""/newline 
l_policy_year/heading="Policy Year"/newline 
*/

/*
end of l_policy_year 
box/noheadings 
""/newline 
"Total for Policy Year"/column=20
l_policy_year/column=50/noheading 
total[lrsdetail:loss_resv]/mask="(ZZZ,ZZZ,ZZZ.99)"/column=100
total[lrsdetail:loss_paid]/mask="(ZZZ,ZZZ,ZZZ.99)" /column=120
total[lrsdetail:lae_resv]/mask="(ZZZ,ZZZ,ZZZ.99)" /column=140
total[lrsdetail:lae_paid]/mask="(ZZZ,ZZZ,ZZZ.99)"/column=160
total[lrsdetail:loss_resv]+total[lrsdetail:loss_paid]+total[lrsdetail:lae_resv]+total[
lrsdetail:lae_paid]/column=180
xob 
*/

end of lrsdetail:claim_no 
box/noheadings/noblanklines 
claim_no /column=1
lrssetup:policy_no/column=20 
lrssetup:agent_no /column=35
lrssetup:line_of_business /column=40
lrssetup:loss_date/column=50
--cppgenerala:class_code /column=60
--cppliabilitya:class_code/column=80
total[lrsdetail:loss_resv]/mask="(ZZZ,ZZZ,ZZZ.99)"/column=100
total[lrsdetail:loss_paid]/mask="(ZZZ,ZZZ,ZZZ.99)" /column=120
total[lrsdetail:lae_resv]/mask="(ZZZ,ZZZ,ZZZ.99)" /column=140
total[lrsdetail:lae_paid]/mask="(ZZZ,ZZZ,ZZZ.99)"/column=160
total[lrsdetail:loss_resv]+total[lrsdetail:loss_paid]+total[lrsdetail:lae_resv]+total[lrsdetail:lae_paid]/column=180
xob 

end of report 
box/noheadings/noblanklines 
total[lrsdetail:loss_resv]/mask="(ZZZ,ZZZ,ZZZ.99)"/column=100
total[lrsdetail:loss_paid]/mask="(ZZZ,ZZZ,ZZZ.99)" /column=120
total[lrsdetail:lae_resv]/mask="(ZZZ,ZZZ,ZZZ.99)" /column=140
total[lrsdetail:lae_paid]/mask="(ZZZ,ZZZ,ZZZ.99)"/column=160
total[lrsdetail:loss_resv]+total[lrsdetail:loss_paid]+total[lrsdetail:lae_resv]+total[lrsdetail:lae_paid]/column=180
xob 

top of page  
enquiryname                   /heading="Report No      "/column=1/newline 
trun(sfscompany:name[1])/column=1/heading="Company Name   "/newline  
trun(str(l_starting_date,"MM/DD/YYYY") + " - " + str(l_ending_date,"MM/DD/YYYY"))/column=1/heading=
"Report Period  "/newline
