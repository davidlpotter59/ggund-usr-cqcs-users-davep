...
l_unearned_type one of 2 then 
    l_premium2 * l_unearned_factor_prior2   -- ((l_total_months_prior2 * 2) -1) divide 24
else
0/decimalplaces=2
define signed ascii number l_dummy = l_premium2 * l_unearned_factor_prior2 

define signed ascii number l_unearned_prior_not_eligable2 = if 
l_unearned_type one of 1 then 
    l_not_eligable_premium2 *  l_unearned_factor_prior2  -- l_current_unearned_factor 
else if 
l_unearned_type one of 2 then 
    l_not_eligable_premium2 * l_unearned_factor_prior2   -- ((l_total_months_prior2 * 2) -1) divide 24
else
0/decimalplaces=2
...
l_unearned_type one of 2 then 
    l_eligable_premium2 * l_unearned_factor_prior2   -- ((l_total_months_prior2 * 2) -1) divide 24
else
0/decimalplaces=2
...
and prsmaster_combined:comm_rate <> 0
--and sfslinea:stmt_lob not one of 999 
and prsmaster_combined:agent_no one of 100
and prsmaster_combined:line_of_business one of 50
and prsmaster_combined:lob_subline one of "70"
list
/nobanner
...
l_days_inforce 
l_total_days 
prsmaster_combined:policy_no 
prsmaster_combined:line_of_business 
prsmaster_combined:lob_subline 
prsmaster_combined:trans_date 
prsmaster_combined:trans_eff 
prsmaster_combined:trans_exp  
prsmaster_combined:premium 
l_unearned
l_unearned_factor
l_current_unearned_factor
l_unearned_type 
...
""/newline
    ...
    Total[l_premium]-total[l_premium1]/column=50
""/newline
    "Percent Change from " + str(year(l_ending_date)) + " - " + str(year(l_ending_date1))
    (Total[l_premium]-total[l_premium1]) divide total[l_premium1]/column=50
""/newline 
xob
top of page  
